...
import {
  actAddWeddingRequest,
  actUpdateWeddingRequest,
} from "./../../action/index";
...
function WeddingForm(props) {
  ...
  const validate = (fieldValues = values) => {
    ...
    //   temp.deposit = fieldValues.deposit
    //     ? +fieldValues.deposit >=
    //       (getCurrentLobby(fieldValues.lobbyId)
    //         ? getCurrentLobby(fieldValues.lobbyId).minUnitPriceTable
    //         : 0)
    //       ? ""
    //       : "Tiền đặt cọc phải lớn hơn hoặc bằng đơn giá bàn tối thiểu"
    //     : "Không được bỏ trống";
    if (
      "weddingDate" in fieldValues &&
      props.currentWeddingState.state === ADD_WEDDING_STATE
    )
      temp.weddingDate = checkDateValidate(fieldValues.weddingDate)
        ? ""
        : "Ngày không hợp lệ";
    if (
      "dateOfOrganization" in fieldValues &&
      props.currentWeddingState.state === ADD_WEDDING_STATE
    )
      temp.dateOfOrganization =
        checkDateValidate(fieldValues.dateOfOrganization) &&
        checkDateOrganizationValidate(
          fieldValues.weddingDate,
          fieldValues.dateOfOrganization
        )
          ? ""
          : "Ngày không hợp lệ";
    setErrors({
  ...
  const checkWeddingExist = () => {
    handleClickVariant(
      "warning",
      "Đã có tiệc cưới tổ chức trong ca và sảnh này!"
    );
  };
  ...
  const clickRowEditWeddingMiddleware = (store) => (next) => (action) => {
    if (action.type === "EDIT_WEDDING_STATE") {
      setValues({
        ...
        oldDate: new Date(props.selectedWedding.dateOfOrganization),
        oldShift: props.selectedWedding.idShift,
        oldLobby: props.selectedWedding.lobbyId,
      });
    }
  ...
  //       props.lobbies.findIndex((lobby) => lobby.id === idLobby)
  //     ];
  //   } else return null;
  // };
      ...
      >
        {moreInfo.timeBegin !== undefined ? (
          <Typography variant="h6" align="center">
            {"Thời gian bắt đầu: " +
              moreInfo.timeBegin.toString() +
              ". Thời gian kết thúc: " +
              moreInfo.timeEnd.toString()}
          </Typography>
        ) : moreInfo.maxTable !== undefined ? (
          <Typography variant="h6" align="center">
            Loại sảnh: {moreInfo.lobbyCategory.name}. Tổng số bàn:{" "}
            {moreInfo.maxTable}. Đơn giá bàn tối thiểu:{" "}
            {minTablePriceUI(moreInfo.minUnitPriceTable)}
          </Typography>
        ) : (
          <Typography variant="h6" align="center">
            ""
        ...
        <Grid container spacing={6} direction="row">
          <Grid item md={6} xs={12} align="center">
            ...
            <Controls.DatePicker
              ...
              value={
                props.currentWeddingState.state === NORMAL
                  ? selectedRowValues.dateOfOrganization
                  : values.dateOfOrganization
              }
              onChange={handleInputChange}
          ...
          <Grid item md={6} xs={12} align="center">
            ...
            />
            <Link
              to={`/wedding/${props.selectedWedding.id}/${
                props.selectedWedding.lobbyId
              }/${
                props.paidBills.some(
                  (feast) => feast.feast.id === props.selectedWedding.id
                )
                  ? "read"
                  : "order"
              }`}
            >
              {props.selectedWedding.id &&
              props.currentWeddingState.state === NORMAL ? (
                <Button
                  id="btnTableService"
                  variant="contained"
...
function convertDateToStringYMD(date) {
  ...
  let result =
    ...
    (month.toString().length === 1
      ...
      : month.toString()) +
    "/" +
    (day.toString().length === 1 ? "0" + day.toString() : day.toString()); // That's your formatted date.
  console.log(result);
  return result;
}
