...
const EnhancedTableToolbar = (props) => {
  ...
  const canUpdateWedding = (permission) =>
    permission.authority === "UPDATE_FEAST";
  ...
  const changeToAddState = () => {
    if (currentWeddingState.state === NORMAL) {
      dispatch(clickRow([]));
      dispatch(addState());
      document.getElementById("formWedding").scrollIntoView({
        behavior: "smooth",
        block: "end",
        inline: "nearest",
      });
  ...
  return (
    <Toolbar className={clsx(classes.root)}>
      ...
      {currentWeddingState.state === NORMAL &&
      privileges.some(canUpdateWedding) ? (
        numSelected > 0 &&
        !props.paidBills.some((feast) => feast.feast.id === selectedRow.id) ? (
          <>
...
function EnhancedTable(props) {
  ...
  const handleClick = (event, id, row) => {
    row = {
      ...row,
      weddingDate: convertDateToStringMDY(row.weddingDate),
      dateOfOrganization: convertDateToStringMDYNew(row.dateOfOrganization),
    };
                  ...
                  {stableSort(rows, getComparator(order, orderBy))
                    ...
                    .map((row, index) => {
                      ...
                      rows[rows.findIndex((r) => r.id === row.id)] = {
                        ...row,
                        status: props.paidBills.some(
                          (feast) => feast.feast.id === row.id
                        )
                          ? true
                          : false,
                      };
                      return (
                        ...
                        >
                          ...
                          <TableCell align="right">
                            {convertDateToStringDMYNew(row.dateOfOrganization)}
                          </TableCell>
