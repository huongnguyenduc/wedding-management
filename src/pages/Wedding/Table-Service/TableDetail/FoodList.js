...
import editState from '../../actions/foodState/edit'
...
const headCells = [
  { id: 'name', numeric: false, disablePadding: true, label: 'Tên món ăn' },
  { id: 'price', numeric: false, disablePadding: false, label: 'Đơn giá' },
  { id: 'foodKind', numeric: false, disablePadding: false, label: 'Loại món' },
...
function FoodList(props) {
  ...
  useEffect(() => {
    if (props.foods)
      setState({...state, data: (props.foods), filterData: (props.foods)});
  }, [props.foods]) //eslint-disable-line
  ...
  const handleClick = (event, name, row) => {
    if (currentFoodState.state !== EDIT_ORDER_FOOD) {
      dispatch(editState());
      dispatch(clickRowFood(row));
    }
    ...
    <Grid container spacing={1} alignItems="flex-end">
          ...
          <Grid item xs={4}>
              <TextField 
                  id="search" 
                  fullWidth 
                  label={"Tìm kiếm theo tên món ăn"}
...
export default connect(null, null)(FoodList);