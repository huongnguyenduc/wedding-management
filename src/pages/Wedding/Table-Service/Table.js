...
function Table(props) {
    ...
    if ("reverseTables" in fieldValues)
      temp.reverseTables =
        checkTables === false
          ...
          ? "Số lượng bàn tối thiểu phải lớn hơn 0"
          : "";
  ...
  const addTableFailure = () => {
    handleClickVariant(
      "error",
      "Lỗi hệ thống. Thêm thông tin đặt bàn thất bại!"
    );
  };
  ...
  const updateTableFailure = () => {
    handleClickVariant(
      "error",
      "Lỗi hệ thống. Chỉnh sửa thông tin đặt bàn thất bại!"
    );
  };
