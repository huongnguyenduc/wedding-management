...
function Calendar(props) {
  ...
  function CalendarHeader() {
    const HeadCells = moment.weekdays().map((item) => {
      return (
        <Grid key={`calHeadCell_${item}`} className={classes.headCell}>
          {item}
        </Grid>
      );
    });
    return <Grid className={classes.calendarHeader}>{HeadCells}</Grid>;
  }
    ...
    return (
      <Toolbar className={classes.toolbar}>
        <Grid className={classes.leftControlButton}>
          <IconButton
            classes={{ root: classes.ToolbarButton, label: classes.ButtonIcon }}
            onClick={DecreaseMonth}
          >
            <ArrowBackIosIcon className={classes.ToolbarButonIcon} />
          </IconButton>
        ...
        >
          <ArrowForwardIosIcon className={classes.ToolbarButonIcon} />
        </IconButton>
      </Toolbar>
    );
  }
  ...
  function checkCurrentDate(date) {
    ...
    let dateObject = Object.assign({}, calendarState.dateObject);
    let tempDateObject = moment(dateObject)
      .set("date", date)
      .format("DD/MM/YYYY");
  ...
  function SelectColor(date1, date2) {
    ...
    if (myOfDate1 === myOfDate2) {
      ...
      else if (difference > 0 && difference <= 7) return "#fe9700";
      else if (difference > 7) return "#4caf50";
      else return "#00bcd4";
    } else if (myOfDate1 < myOfDate2) {
      return "#00bcd4";
    }
    return "#4caf50";
  }
  ...
  function GridStatus(props) {
    ...
    return (
      <Grid>
        <Typography
          className={classes.GridFeastInfo}
  ...
  function CalendarBody() {
    let lastMonth = moment(calendarState.dateObject).set(
      "month",
      parseInt(month()) - 2
    );
    let lastDateOfLastMonth =
      parseInt(lastMonth.endOf("month").format("D")) - firstDayOfMonth() + 1;

    let emptyCells = [];
    for (let i = 0; i < firstDayOfMonth(); i++) {
      emptyCells.push(
        <Grid key={`emptyCell_${i}`} className={`${classes.tableCell}`}>
          <Typography className={`${classes.txtDateLastMonth}`}>
            {lastDateOfLastMonth}
          </Typography>
        </Grid>
      );
    ...
    for (let i = 1; i <= daysInMonth(); i++) {
      ...
      DayCells.push(
        <Grid
          key={`cell_${i}`}
          className={`${
            isCurrenDate ? classes.cellCurrentDay : classes.tableCell
          }`}
        >
          <Typography
            className={`${
              isCurrenDate ? classes.txtCurrentDay : classes.txtDate
            }`}
          >
            {i}
          </Typography>
    ...
    allCells.forEach((cell, index) => {
      ...
      if (index === allCells.length - 1) {
        ...
        for (let i = lastCells; i < 7; i++)
          Cells.push(
            <Grid key={`empty_${i}`} className={classes.tableCell}>
              <Typography className={`${classes.txtDateLastMonth}`}>{`${
                i - lastCells + 1
              }`}</Typography>
            </Grid>
  ...
  return (
    <Grid className={classes.CalendarPage}>
      ...
      <Container maxWidth="lg" className={classes.NoteContainer}>
        <Typography className={`${classes.TfNote} ${classes.BlueNote}`}>
          Đã tổ chức
        </Typography>
        <Typography className={`${classes.TfNote} ${classes.RedNote}`}>
          Đến ngày tổ chức
        </Typography>
        <Typography className={`${classes.TfNote} ${classes.YelowNote}`}>
          Sắp tổ chức
        </Typography>
        <Typography className={`${classes.TfNote} ${classes.GreenNote}`}>
          Chưa tổ chức
        </Typography>
      </Container>
    </Grid>
...
function MonthSelector(props) {
  ...
  const monthCells = months.map((item, index) => {
    return (
      <Grid
        ...
        className={`${classes.SelectGirdItem} ${
          item === currentMonth ? classes.CurrentItem : ""
        }`}
        name={index}
        onClick={onMonthClick}
      >
        <Typography name={index} className={classes.SelectItem}>
          {item}
        </Typography>
      </Grid>
    );
  ...
  return (
    <Grid className={`${classes.monthsContainer} ${className}`} {...other}>
      {monthCells}
    </Grid>
  );
}
...
function YearSelector(props) {
  const { setYear, currentYear, selectMonth, onClose, className, ...orther } =
    props;
  ...
  for (let i = CurrentYear - 4; i <= CurrentYear + 4; i++)
    yearCells.push(
      <Grid
        ...
        key={`yearItem${i}`}
        className={`${classes.SelectGirdItem} ${
          i === CurrentYear ? classes.CurrentItem : ""
        }`}
        name={i}
        onClick={onYearClick}
      >
        <Typography name={i} className={classes.SelectItem}>
          {i}
        </Typography>
      </Grid>
    );
  ...
  return (
    <Grid className={`${classes.yearsContainer} ${className}`} {...orther}>
      {yearCells}
    </Grid>
  );
}
