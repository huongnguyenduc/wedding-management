...
import { actSelectFood } from "../actions/actions";
  ...
  return (
    <Grid item xs={12} sm={6} md={4} lg={4} className={classes.Card}>
      <Grid className={classes.Content}>
        ...
        <link
          href="https://fonts.googleapis.com/css2?family=Pacifico&family=Patrick+Hand&family=Pattaya&display=swap"
          rel="stylesheet"
        />
        <Grid className={`CardMedia ${classes.CardMedia}`}>
          <CardMedia className={`Image ${classes.Image}`} image={data.img} />

          <IconButton
            aria-label="edit"
            onClick={onEditClick}
            classes={{ label: classes.ButtonLabel }}
            className={`ButtonGroup ${classes.ButtonGroup}`}
          >
