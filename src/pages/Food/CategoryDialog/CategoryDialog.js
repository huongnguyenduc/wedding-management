  ...
  return (
    <TableHead>
      <TableRow>
        ...
        <TableCell label="cellControl"></TableCell>
      </TableRow>
    </TableHead>
  );
}
...
function CategoryDialog(props) {
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, categoryData.length - page * rowsPerPage);
          ...
          <Table>
            ...
            <TableBody>
              {stableSort(categoryData, getComparator(order, orderBy))
                ...
                .map((row) => {
                  return <Row key={`cate_${row.id}`} category={row} />;
                })}
...
function Row(props) {
  ...
  function CancelHandler() {
    ...
    else setRowState({ id: "", name: "", moreInfo: "", editing: false });
  }

  function success() {
    if (category) setRowState({ ...rowState, editing: false });
    else setRowState({ id: "", name: "", moreInfo: "", editing: false });
  }

  const checkExist = () => {
    const find = categoryData.find(
      (item) =>
  ...
  function FinishHandler() {
    if (rowState.name && rowState.moreInfo) {
      ...
      else dispatch(InsertFoodCategory(rowState, success));
    } else {
