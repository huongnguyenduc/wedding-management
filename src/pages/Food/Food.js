...
function Food() {
  ...
  const finalData = FoodData.filter((item) => {
    return (
      keyword === "" ||
  ...
  return (
    <MuiThemeProvider theme={theme}>
      <div>
        ...
        {FoodCategory.map((group, index) => {
          return (
            <Element
              key={group.id}
              name={`category_tab_${group.id}`}
              style={{
                paddingTop: index === 0 ? "200px" : "100px",
                paddingBottom: "100px",
              }}
            >
              <FoodGroup
        ...
        {privileges.some(canUpdateFood) ? (
          <div className={classes.buttonArea}>
            <Fab
              color="primary"
              aria-label="add"
              variant="extended"
              onClick={handlefooDialogOpen}
            ...
            <Fab
              ...
              classes={{ root: classes.MenuFab, label: classes.ButtonLabel }}
            >
              <RestaurantMenuIcon />
              Quản lý loại món
            </Fab>
      ...
      {fooDialogOpen ? (
        <FoodDialog handleClose={handleFoodDialogClose}></FoodDialog>
      ) : null}
