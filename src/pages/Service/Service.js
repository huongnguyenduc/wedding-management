...
function Service() {
  ...
  const Filer = (service) => {
    ...
    if (str.search(key) !== -1 || service.price == keyWord) return true;
    else return false;
  };
  ...
  const canUpdateService = (permission) =>
    permission.authority === "UPDATE_SERVICE";
  return (
    <Container className={classes.ServicePage}>
      ...
      {privileges.some(canUpdateService) ? (
        <Fab
          color="primary"
          aria-label="add"
          variant="extended"
          onClick={openInsertService}
          className={`insertFab ${classes.InsertFab}`}
          classes={{ label: classes.ButtonLabel }}
        >
      ...
      {openInsert ? (
        <ServiceDialog closeHandler={() => setOpenInsert(false)} />
      ) : null}
      <Snackbar
        open={error.open}
      ...
      >
        <Alert open={error.open} severity={error.severity} onClose={CloseAlert}>
          {error.message}
        </Alert>
      </Snackbar>
      <Snackbar
