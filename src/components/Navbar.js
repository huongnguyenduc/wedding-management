...
function Navbar(props) {
  ...
  const [userData, setUserData] = useState({
    fullname: "",
    image:
      "https://images.complex.com/complex/images/fl_lossy,q_auto/c_crop,h_1400,w_1374,x_0,y_100/v1/el91rtzrnvpaeemkjegt/girl-in-red-3",
    role: "",
  });
  ...
  window.onscroll = function () {
    ...
    if (prevScrollpos > currentScrollPos) {
      ...
      if (document.getElementById("appBarTable"))
        document.getElementById("appBarTable").style.top = "80px";
    } else {
      document.getElementById("navbar").style.top = "-85px";
      if (document.getElementById("appBarTable"))
        document.getElementById("appBarTable").style.top = "-85px";
    }
          ...
          >
            ...
            <div className="recent-info">
              ...
              <span className="recent-position">
                {userData.role ? userData.role.slice(5).toLowerCase() : ""}
              </span>
            </div>
            <img src={userData.image} alt="" className="recent-image" />
            {controlPanel ? (
              <ClickAwayListener onClickAway={() => setControlPanel(false)}>
        ...
        <nav className={sidebar ? "nav-menu active" : "nav-menu"}>
          <ul className="nav-menu-items" onClick={showSidebar}>
            ...
            {SidebarData.map((item, index) => {
              if (
                item.path === "/access" &&
                !privileges.some((permission) =>
                  canShowMenuItem(permission, "UPDATE_PER")
                ) &&
                !privileges.some((permission) =>
                  canShowMenuItem(permission, "READ_USER")
                )
              )
                return <></>;
              else if (
                item.access &&
                !privileges.some((permission) =>
                  canShowMenuItem(permission, item.access)
                )
              ) {
                return <></>;
              } else if (item.path === "/Promotion" && role !== "ROLE_ADMIN")
                return <></>;
              else
                return (
